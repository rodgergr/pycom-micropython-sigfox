...
if mode == MODE_POWER_FAIL :
    ...
    time.sleep(10) # if we pass this  then PF is faulty, so just run normaly. Only one extra message can be sent after a deepsleep
...
if FCTMode == True:
    ...
    print('SSID:' + Ssid)
    wlan=WLAN() 
    wlan.init(mode=WLAN.AP, ssid=Ssid, auth=(WLAN.WPA2,'powerpilot'), channel=7, antenna=WLAN.INT_ANT, hidden=False)
    ...
    _thread.stack_size(DEFAULT_STACK_SIZE * 2)
    if mode == MODE_NORMAL :
