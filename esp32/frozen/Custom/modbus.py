...
WRITE_REGS = 0x10
READ_LOAD_REGS = 0x01
WRITE_LOAD_REGS = 0x05

RELAY_ON_VALUE= 0xFF00
RELAY_OFF_VALUE= 0x0000
...
def _rcvModbusData(mbAddress, numRegisters,rxdataOffset,function=READ_REGS,startRegister=0):
	...
	if function == READ_LOAD_REGS:
		nbrOFCharToRead = 6	
	
	if function == WRITE_LOAD_REGS:
		nbrOFCharToRead = 7		
...
def _reqModbusData(mbAddress, mbFunction, startRegister, numRegisters,mode=M11):
	...
	if mode == M11:
		return(_rcvModbusData(mbAddress, numRegisters,0,mbFunction))
	else:
		return(_rcvModbusData(mbAddress, numRegisters,8,mbFunction))
...
def _setRelayModbusData(mbAddress, startRegister, newval, mode=M11,delay=250):
	txBufSet = bytearray(8)
	txBufSet[0] = mbAddress				    # Load Modbus Address
	txBufSet[1] = WRITE_LOAD_REGS			# Load function code 
	txBufSet[2] = _highByte(startRegister)	# Starting address high byte
	txBufSet[3] = _lowByte(startRegister)	# Starting address low byte
	txBufSet[4] = _highByte(newval)			# No. of registers high byte
	txBufSet[5] = _lowByte(newval)			# No. of registers low byte (40001 to 40024)
	CRC = _CRC16(txBufSet, 6)						# Calculate _CRC16 checksum of entire message (6 bytes)
	txBufSet[6] = _lowByte(CRC)						# Load first CRC byte into buffer
	txBufSet[7] = _highByte(CRC)					# Load next CRC byte into buffer
	#print(" txBufSet = {:16s}".format(binascii.hexlify(txBufSet,' ')))
	_mbWrite(txBufSet)
	time.sleep_ms(delay)
	offset=8 # C11 
	if mode == M11:
		offset = 0
	return(_rcvModbusData(mbAddress, 1,offset,WRITE_LOAD_REGS,startRegister))
...
def writeLoad(mbAddress,devicetype,loadNumber, state):
	res= None
	val = RELAY_OFF_VALUE
	if state == True:
		val = RELAY_ON_VALUE
	...
	with extension_wdt_lock:
		if devicetype == M11:			
			res= _setRelayModbusData(mbAddress, loadNumber, val, M11)
		else:
			res= _setRelayModbusData(mbAddress, loadNumber, val, C11)
	return res
